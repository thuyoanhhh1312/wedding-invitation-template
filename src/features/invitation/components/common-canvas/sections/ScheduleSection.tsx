...
export function ScheduleSection({ content, model }: ScheduleSectionProps) {
  return (
    <section className='relative overflow-visible bg-[#5d1b18] px-8 pb-0 pt-8 text-[#e7d7c5]'>
      <div className='relative min-h-180'>
        ...
        <div className='absolute left-[67%] top-[13.6rem] -translate-x-1/2 space-y-1.5 text-center font-formal text-[0.96rem] leading-[1.15rem] tracking-[0.015em] text-[#ead8c4]'>
          <p className='whitespace-nowrap'>{content.eventDate.displayShort}</p>
        ...
        <div className='pt-94'>
          <img
            ...
            loading='lazy'
            src={commonInvitationAssets.commonScheduleTitle}
          />
        </div>

        <div className='relative mt-8 min-h-56'>
          <div className='ml-7 max-w-45 text-left font-display text-[1rem] leading-6 tracking-[0.03em] text-[#f0e2d4]'>
            <p>{model.scheduleSummaryText}</p>
          </div>
          <img
            alt=''
            aria-hidden='true'
            className='absolute bottom-0 right-5 h-68 w-auto opacity-95'
            loading='lazy'
            src={commonInvitationAssets.flowerTransparent}
          />
        </div>
      </div>
      ...
      <div className='relative z-10 mt-10 -mb-16'>
        <img
          ...
          loading='lazy'
          src={commonInvitationAssets.chandelierCouple}
        />
      </div>
    </section>
  );
